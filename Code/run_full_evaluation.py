...
# CANONICAL CURRENT-MANUSCRIPT EVALUATION.
...
#
# Do not confuse these outputs with the historical 70/30 calibration files
# named results/mozilla_results*.csv. See ARTIFACT_MANIFEST.md.
#
# Usage:
...
if __name__ == "__main__":
    ...
    # Protect the frozen canonical outputs from accidental partial reruns. A
    # canonical run requires all five methods and the exact two study subsets.
    input_errors = []
    if not os.path.isfile(ALERTS_CSV):
        input_errors.append(f"missing alert file: {ALERTS_CSV}")
    if not TORCH_OK:
        input_errors.append("PyTorch is unavailable; Transformer cannot run")

    expected_files = {"firefox-android": 342, "mozilla-beta": 1477}
    for ds_name, _, ts_subfolder in DATASETS:
        ts_dir = os.path.join(TS_BASE, ts_subfolder)
        n_files = len(glob.glob(os.path.join(ts_dir, "*_timeseries_data.csv")))
        if n_files != expected_files[ds_name]:
            input_errors.append(
                f"{ds_name}: expected {expected_files[ds_name]} time-series "
                f"files, found {n_files} in {ts_dir}"
            )

    if input_errors:
        print("ERROR: canonical evaluation inputs are incomplete:")
        for message in input_errors:
            print(f"  - {message}")
        print("No canonical result file was modified.")
        sys.exit(2)
