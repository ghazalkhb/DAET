...
# CALIBRATION/SUPPLEMENTARY ANALYSIS (70/30 firefox-android), not the canonical
# two-dataset 60/20/20 evaluation. See ARTIFACT_MANIFEST.md.
# End-to-End Replay Experiment -- Mozilla Telemetry
#
# Simulates adaptive "trace-on" decisions by replaying the detector outputs
# already produced by the historical calibration pipeline. For each signature
# × predictor:
#
...
#
# Relies on pre-computed outputs from the historical calibration pipeline:
#   results/mozilla_results*.csv -- raw per-step detector outputs
#   results/vectors/<sig>_meta.json -- push timestamps (not distributed here)
...
# ── Settings (must match the frozen calibration outputs) ────────────────────
...
# ── Raw result CSVs (produced by the historical calibration detectors) ───────
RAW_CSVS = {
...
if missing_methods:
    ...
    printf("       Restore the historical calibration outputs first.\n")
    if not raw:
...
for method in METHODS_AVAIL:
    ...
    # and historical calibration outputs)
...
pdf.body_text(
    ...
    "pre-computed calibration detector CSVs (mozilla_results*.csv). All "
    "random seeds are fixed by the deterministic nature "
