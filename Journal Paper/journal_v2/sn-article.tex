%% Springer Nature journal paper (sn-jnl class — sn-nature style)
%% Journal: Empirical Software Engineering / Software Quality Journal
...
\title[Adaptive Execution Tracing with Large-Scale Validation]{Towards Efficient Performance Debugging: A Dynamic and Adaptive Execution Tracing Technique with Large-Scale Comparative Validation}

\author[1]{\fnm{Mohammed} \sur{Adib Khan}}\email{khanm@brocku.ca}\equalcont{These authors contributed equally to this work.}
\author*[1]{\fnm{Morteza} \sur{Noferesti}}\email{mnoferesti@brocku.ca}\equalcont{These authors contributed equally to this work.}
\author[1]{\fnm{Naser} \sur{Ezzati-Jivan}}\email{nezzatijivan@brocku.ca}\equalcont{These authors contributed equally to this work.}

\affil*[1]{\orgdiv{Computer Science Department}, \orgname{Brock University},
  \orgaddress{\street{1812 Sir Isaac Brock Way}, \city{St. Catharines}, \postcode{L2S-3A1}, \state{ON}, \country{Canada}}}
...
(1)~Tracing-Function Selection via call-stack profiling and coefficient-of-variation (CoV) ranking,
(2)~Change Detection based on anomaly scoring with a dynamic threshold, and
(3)~Trace Configuration Adjustment that activates tracepoints only when the anomaly score
exceeds a tolerance-based gate.

This journal extension provides a large-scale, statistically grounded evaluation
of multiple change-detection engines (ARIMA, LAST, SMA, EWMA, and a compact Transformer)
across two Mozilla performance-telemetry benchmarks under a unified 60\%/20\%/20\%
chronological protocol, including dual-dataset validation, replay-based deployment analysis,
and a fair multi-seed Transformer comparison.
...
operating point, while alternative settings trade recall for more selective detection.
...
\keywords{kernel tracing, application tracing, execution tracing,
          adaptive performance monitoring, performance debugging,
          anomaly detection, ARIMA, change detection, Mozilla telemetry}
...
\begin{enumerate}
  ...
  \item An ablation study sweeping $\beta \in \{0.1,0.2,0.3,0.5,0.7\}$,
    ...
    five ARIMA orders, and six SMA windows, characterising the stable operating
    regions of the parameter space and the operational tradeoffs involved
    in parameter selection.
  \item A trace-on replay experiment on the full test-set chronology for the
    four statistical detectors, measuring
    storage reduction (89-91\%), trigger frequency ($\approx$5.7-6.1\%),
    and alert coverage (70-93\%), plus an efficiency-frontier
    analysis characterising the storage--coverage Pareto trade-off.
...
All reported values measure \emph{agreement with Treeherder alerts}, not
ground-truth anomaly validity: Treeherder may miss transient regressions
or include false positives from infrastructure changes, and many DAET
detections that appear as false positives may correspond to genuine but
unlabelled performance excursions.
...
all ten pairwise comparisons ($\alpha = 0.05$).
The upper block covers the six pairs among the four statistical detectors.
...
However, the EWMA validation curves are \emph{monotonically decreasing}
on both datasets: F1 is highest at $\alpha=0.05$ and declines at every
higher $\alpha$ value, meaning $\alpha=0.05$ is the \emph{lower boundary}
of the tested grid ($\{0.05, 0.10, \ldots, 0.50\}$), not an interior
optimum.
The true optimal $\alpha$ may therefore be smaller than 0.05; the results
represent the best performance achievable within the tested grid.
This is an acknowledged limitation of the canonical evaluation.
...
\begin{figure}[tbp]
  ...
  \\[6pt]
  \begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=\textwidth]{images/three_way/fa_val_tuning_ewma.png}
    \caption{Firefox-Android EWMA: peaks at $\alpha=0.05$.}
    \label{fig:tuning_fa_ewma}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=\textwidth]{images/three_way/mb_val_tuning_ewma.png}
    \caption{mozilla-beta EWMA: peaks at $\alpha=0.05$.}
  ...
  \caption{Validation-set parameter tuning curves for SMA (top row) and EWMA
    ...
    selection.
    EWMA curves are monotonically decreasing on both datasets: the optimum
    lies at the lower grid boundary $\alpha=0.05$, so a finer grid at
    smaller $\alpha$ values may yield further improvement.}
...
\begin{enumerate}
  ...
  \item \textbf{EWMA at the lower grid boundary matches SMA in F1.}
    ...
    Both datasets select $\alpha=0.05$, the lowest value in the tuning grid
    $\{0.05, 0.10, \ldots, 0.50\}$; the validation curve is monotonically
    decreasing, meaning a smaller $\alpha$ might further improve F1.
    The current results therefore represent EWMA's best-within-grid
    performance rather than a confirmed global optimum.
...
representative 20-signature subset.
...
\midrule
(1,0,0) & 0.057 & 0.450 & 0.100 & 0.065 \\
(0,1,1) & 0.087 & 0.550 & 0.147 & 0.055 \\
(1,1,0) & 0.057 & 0.450 & 0.099 & 0.058 \\
\textbf{(1,1,1)} & \textbf{0.094} & \textbf{0.550} & \textbf{0.158} & \textbf{0.054} \\
(2,1,2) & 0.091 & 0.550 & 0.153 & 0.054 \\
\bottomrule
\end{tabular}
\caption{ARIMA order ablation (20-signature subset).
  Bold marks the chosen default.}
\label{tab:ablation_order}
\end{table}

ARIMA(1,1,1) achieves the best F1 on this subset.
Adding MA terms to the pure AR(1) model (comparing (1,1,0) to (1,1,1))
improves both precision and F1 without materially increasing FAR.
The more complex (2,1,2) model provides no statistically meaningful gain.
Note that these results derive from a 20-signature subset; they are
used only to justify the order selection and should not be over-interpreted
as evidence of general superiority across the full 214-signature benchmark.
...
$\beta=0.20$ or $\theta=0.40$ for ARIMA, and the ARIMA-order result
should be treated with caution as it derives from a 20-signature subset.
...
ARIMA(1,1,1).
It significantly outperforms LAST and EWMA, matches SMA in F1 and precision,
and provides a statistically motivated prediction model that distinguishes
...
Firefox-Android.
With tuned $\alpha=0.05$ (Section~\ref{sec:three_way}), EWMA achieves
...
\subsection{Precision Ceiling}

Precision is low ($<0.12$) for all methods.
This is inherent to the dataset: Treeherder regression alerts are based on
a different detection algorithm (Bayesian change-point detection), and many
short performance excursions that DAET detects are \emph{real} performance
anomalies that simply did not persist long enough for Mozilla's system to
raise a formal regression ticket.
The metric captures \emph{how many of our detection windows coincide with
official alerts}, not whether the detected anomalies are genuine.
We therefore distinguish two precision concepts: ``Treeherder agreement
precision'' (what Tables~\ref{tab:main_results}--\ref{tab:three_way_mb}
report) and ``true anomaly precision'' (the fraction of detections
corresponding to genuine performance anomalies).
The true anomaly precision is unknown without manual inspection.
A stronger validation would require (i)~manual inspection of a random sample
of alleged false positives to classify them as genuine anomalies vs.~noise,
or (ii)~cross-referencing with a second alert source (e.g., Bugzilla regression
tickets) to identify detections that correspond to real events not yet labelled
by Treeherder.
We acknowledge this as a limitation and note it as directions for future work.

\subsection{Scientific Conclusions vs.\ Deployment Recommendations}

It is important to distinguish what the evidence supports scientifically
from what it supports as a deployment recommendation.

\textbf{What the evidence shows}:
All metrics throughout this paper measure \emph{Treeherder-agreement}:
how well a detector's binary output aligns with Mozilla's automated Bayesian
alert system.
This is not the same as measuring true anomaly detection accuracy or
debugging effectiveness.
Subject to that construct constraint:
the canonical evaluation (Section~\ref{sec:three_way}) shows that SMA and
properly tuned EWMA lead on Treeherder-agreement F1 on two repositories
under a fair 60\%/20\%/20\% protocol; ARIMA is third; LAST is weakest.
The calibration baseline (Section~\ref{sec:largescale:RQ1}) confirms the
ranking on a 70/30 split: SMA(w=20) reaches recall~0.657, F1~0.186, and
detection rate~96.0\%; ARIMA is statistically indistinguishable from SMA
in F1 ($p=0.547$) and precision ($p=0.652$).
The compact Transformer under the canonical protocol achieves
F1~=~0.105~($\pm$0.002) on Firefox-Android and F1~=~0.079~($\pm$0.001) on
mozilla-beta, showing that a learned backbone is competitive with classical
methods on this alert-agreement benchmark.

\textbf{What the evidence does not show}: The evidence does not show that
any single method is best for all workloads or all deployment contexts.
The benchmark is limited to one application (Firefox-Android), one
telemetry platform (Treeherder), and one operating period.
Generalisation to other systems, seasonality regimes, or alert-generation
protocols requires further evaluation.

\textbf{Deployment recommendation}:
For deployments where recall and alert coverage are the primary objectives,
SMA is the recommended default.
Under the canonical 60\%/20\%/20\% protocol, SMA($w=40$) achieves F1~=~0.104
and detection rate 72.8\% on Firefox-Android.
Under the calibration baseline (for deployments matching the conference-paper
setup), SMA($w=20$) achieves recall~0.657, F1~0.186, and detection rate 96.0\%
(30~s runtime, $16\times$ less memory than ARIMA).
For a learned predictor, the compact Transformer under the canonical protocol
is the recommended choice (F1~=~0.105 on Firefox-Android, 0.079 on
mozilla-beta).
ARIMA(1,1,1) is appropriate when explicit trend modelling, interpretable
residuals, or extensibility to higher-order variants are required;
its 27$\times$ runtime overhead (Table~\ref{tab:compute_cost}) should be
...
Second, the EWMA validation curve is monotonically decreasing on both
datasets, meaning the selected $\alpha=0.05$ sits at the lower boundary of
the tested grid; the true optimal $\alpha$ may be smaller, and the
reported EWMA results are best-within-grid rather than globally optimal.
...
A critical limitation is that this paper evaluates \emph{agreement with
Treeherder alerts}, not improvement in actual performance-debugging
effectiveness.
Precision, recall, and F1 are computed relative to Treeherder's Bayesian
change-point alerts, which are themselves imperfect: they may miss
transient regressions, include false positives from infrastructure changes,
and are generated by a different detection algorithm with different
sensitivity settings.
Consequently, the paper does not show that DAET improves engineer
productivity or reduces time-to-root-cause---it shows that DAET's
binary output is correlated with an existing automated alerting system.
Whether higher Treeherder-agreement precision corresponds to genuine
improvement in debugging utility would require a controlled user study or
a longitudinal analysis linking DAET-triggered traces to bug-fix time.
The tolerance window $\tau = 5$ samples introduces further matching
leniency.
All four statistical detectors use the same evaluation protocol, so the \emph{comparative}
findings are unaffected by this construct choice, but all absolute
precision and recall values should be interpreted as
``Treeherder-agreement metrics'' rather than true anomaly precision
(see the Precision Ceiling discussion in Section~\ref{sec:discussion}).
The false-positive counts have not been manually adjudicated: a detection
event classified as a false positive by Treeherder alignment may correspond
to a genuine but unlabelled performance excursion.
A stricter validation would require manual inspection of a random sample of
alleged false positives, or cross-referencing with a second alert source
(e.g., Bugzilla regression tickets).
...
The canonical evaluation (Section~\ref{sec:three_way},
\texttt{run\_full\_evaluation.py}) showed that on Firefox-Android
(60\%/20\%/20\%, independent validation tuning) SMA($w=40$) achieves F1~=~0.104
and detection rate 72.8\%; EWMA($\alpha=0.05$) matches it at F1~=~0.103 with
marginally higher storage reduction (89.4\%).
On mozilla-beta (1{,}477 signatures), SMA($w=20$) leads at F1~=~0.085 and
detection rate 91.7\%; EWMA achieves F1~=~0.083.
The qualitative ranking (SMA~$\approx$~EWMA~$\geq$~ARIMA~$>$~LAST) is
consistent across both datasets on the Treeherder-agreement benchmark,
providing preliminary evidence of robustness within the Mozilla CI context;
stronger generalisability claims require validation against an independent
ground truth.
The compact Transformer (five seeds, canonical protocol,
dataset-specific HP tuning) achieves F1~=~0.105~($\pm$0.002) and
detection rate 74.4\% on Firefox-Android---competitive with tuned
SMA---and F1~=~0.079~($\pm$0.001) on mozilla-beta.
The calibration baseline (Section~\ref{sec:largescale:RQ1}, 70\%/30\%,
214 signatures) found SMA(w=20) achieving recall~0.657 and F1~0.186;
statistical tests confirm ARIMA is indistinguishable from SMA in F1
($p=0.547$) and precision ($p=0.652$), and significantly outperforms
LAST and EWMA in precision, recall, and F1 (all $p\leq 0.002$;
ARIMA--LAST FAR difference not significant, $p=0.765$).
ARIMA(1,1,1) is an appropriate alternative when explicit trend modelling,
interpretable residuals, or extensibility to higher-order ARIMA variants
are operationally required; at 27$\times$ the runtime cost and 16$\times$
the memory footprint of SMA (Table~\ref{tab:compute_cost}), it should not
be used as the default engine on the basis of measured accuracy alone.
The replay experiment (Section~\ref{sec:replay}) confirmed that all four
statistical detectors reduce trace storage by 89--91\% on the
calibration-baseline test split, with SMA achieving the highest mean
per-signature alert recall (93.0\%) and EWMA the highest storage reduction
(91.0\%).
The ablation study (Section~\ref{sec:ablation}) confirmed that the default
parameters ($\beta = 0.30$, $\theta = 0.30$, ARIMA(1,1,1), $\tau = 5$)
are an operationally justified high-recall operating point, not globally
optimal settings; under the ablation's stricter regression-only GT,
$\beta=0.20$ yields ARIMA F1~=~0.266 vs.\ the default 0.144 (vs.\ 0.174
under the main-table GT), and $\theta\geq0.40$ yields F1~=~0.27 at the
...
(ii)~validating the low-precision findings against a manually annotated
subset of alleged false positives and against Bugzilla regression reports
to estimate true anomaly precision;
