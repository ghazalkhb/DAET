...
\subsection{Outline}
The paper is structured into four sections. Section \ref{rwork} reviews relevant research in the field of adaptive tracing. Section \ref{methodology} details the proposed methodology for adaptive tracing, including metric selection, trend detection, and anomaly scoring. Section \ref{results} presents four case studies applied to major open-source applications, demonstrating the effectiveness of the proposed approach. Finally, Section \ref{conc} provides conclusions, discusses limitations, and outlines directions for future work.
...
Accurately tracing an application with a large number of function calls requires selecting a distinct set of functions for each task. To address RQ1RQ1, we introduce the Tracing-Function Selection phase, which comprises four key steps.
    ...
    \ForEach{$Snapshot \in CallStackSnapshots$}
        ...
        \ForEach{$Family \in FamilyGroup$}
            ...
            \Else
                ...
                \State $FamilyGroupCount[family_{index}] =+ 1$
            \EndIf
        \EndFor
    \EndFor
...
\begin{algorithm}[tbp]
  ...
  \begin{algorithmic}[1]
    ...
    \ForEach{$MC \in FunctionsExecTimeList$}
        \State $\sigma =$ Calculate Mean Of $MC$
        \State $\mu =$ Calculate Standard Deviation Of $MC$
...
\begin{algorithm}[tbp]
  ...
  \begin{algorithmic}[1]
    ...
    \Function{Change\_Detection}{$FunctionExecutionTimeDataset$,\newline$Threshold_{arima}$,$\beta$,$Threshold_{anomaly}$}
        ...
        \State $T_{predict} = Predict\_T(MetricData)$
        ...
        \Else
            \State $isAnomaly=False$
        \EndIf
        \State $Anomaly\_Score(isAnomaly,\beta)$
        \If{$anomalyScore >= Threshold_{anomaly}$}
...
To dynamically adjust tracing configurations based on detected changes, a decision mechanism has been implemented. This mechanism relies on two factors: (1) change detection through ARIMA models and (2) a pre-determined acceptable threshold for deviations from the original trend. This decision mechanism addresses RQ3RQ3 by ensuring that tracing configurations are adjusted in response to significant changes, enabling proactive performance monitoring.

System-level tracing is achieved by controlling tracing components that record system-level events and system calls. Similar to the application-level approach, a list of system calls made during various loads and stress phases is compiled, sorted, and ranked. The execution time for each system call is monitored, and if abnormalities or trend changes are detected, a Linux system call table (e.g., \cite{valsorda-no-date}) is used to map the system call to the corresponding kernel-level component. For example, the system call sendmsgsendmsg is handled by the kernel component net/socket.cnet/socket.c, which pertains to the network subsystem. By identifying the relevant kernel-level component, tracing instruments can be enabled to record events related to that component, such as network-related events in this case.
...
Following the Bugzilla report \cite{clessili-2022}, we have identified the culprit function "$mozilla::VsyncRefreshDriverTimer::RunRefreshDrivers$" which caused the delay. The culprit function had a ranking of 14 on the list. Firefox used this function call to maintain synchronization between the frame refresh rate of the monitor and Firefox itself to minimize tearing effects when the PDF is scrolled. We successfully identified the problematic function call out of eight thousand function calls by following only four hundred function calls which is a huge overhead reduction compared to tracing all the functions.

However, our approach further reduces the overhead by not only specifying which function to trace but also by adaptively deciding when to trace the functions. Therefore, not only did it save overhead from tracing all the functions, but it also saved system resources from selectively tracing the functions only when needed.
...
In order to evaluate this case study, looking into the usefulness of the metric selection is optional as the function itself is the performance issue culprit and being able to pick up the culprit function from a list of eight thousand function calls itself shows the usefulness of the proposed approach. Processing overhead calculation for this case study is very relative to how the application will be used in an actual use case scenario. Since it takes as little as 158 nanoseconds to materialize an LTTng tracepoint, \cite{gebai-2018}; therefore, the overhead for 410 tracepoints is very negligible. However, the overhead for storing the trace data is more concerning than materializing the tracepoint itself. Table \ref{tableevaluationcs21} shows the evaluation criteria and the overhead being saved through the use of the proposed approach for adaptive tracing. 
...
Our proposed approach successfully captured the culprit function call and found the class name of the function call from the call stack. It was an exact match to that of the Bugzilla report.
...
As mentioned in Table \ref{tableevaluationcs41}, the culprit system call, $sendmsg$ could be looked up in a Linux system call table \cite{valsorda-no-date} to find out the kernel component of that system call. Looking up the $sendmsg$ system call from the table, we can find that $net/socket.c$ handles that system call. This component is solely responsible for all network-related tasks in a Linux system. Therefore, we could use this information to trace network-related events adaptively. As a result, this case study has successfully demonstrated that we can perform multi-level (system and application level) adaptive tracing with our proposed approach.
...
We have faced a few challenges during the implementation stage of our work. Firstly, the threshold values to define an acceptable fluctuation away from ARIMA as well as the anomaly score's $\beta$ value had to be manually tested around. Therefore, these values became system and application specific. The threshold value and the $\beta$ value found from one system wouldn't make sense in another given that they serve different tasks. Secondly, it was noticed that functions which had execution time anomalies that showed straight spike and then a drop back down followed by normal execution for a while and then spike again on repeat, had trouble recognizing these anomalies to be traced. This was due to the fact that anomaly score's threshold kept on getting reset after the spike and the distance from one spike to the next was too long for the anomaly score value to hold. Therefore, it never reached or broken out of the threshold for one single spike. Third, it was also a challenge to decide on the function list cutoff number. For our case studies, we have used a hard coded 5\%, however, in much more complex systems this might not be enough. Finally, the depth of family-groups to enable or disable tracing was hard coded to be 20, however, this might cause excess data collection or much lesser data collection depending on the complexity of the system.


\section{Conclusion and Future Work}\label{conc}
This paper introduces an adaptive tracing approach to address challenges in diagnosing performance issues. Leveraging autoregressive integrated moving average (ARIMA) models, the approach predicts system and application-level metrics, detecting anomalies by identifying deviations from past trends as indicators of potential performance issues. The adaptive tracing mechanism effectively balances the trade-off between filtering non-significant anomalies and capturing critical ones. Case studies demonstrate its ability to identify application issues, pinpoint problematic classes and functions, and highlight kernel-level components affecting performance.
