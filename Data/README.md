# Data used by the current evaluation

The current manuscript evaluates the `firefox-android` (342 signatures) and
`mozilla-beta` (1,477 signatures) subsets. `alerts_data.csv` and `bugs_data.csv`
are included here; the large per-signature measurements are not. For a full
rerun, restore them under `Data/timeseries-data/firefox-android/` and
`Data/timeseries-data/mozilla-beta/` with filenames of the form
`<signature_id>_timeseries_data.csv`.

The remaining sections document the broader source dataset and the collection
scripts from which the study subsets were derived. References to additional
repositories or 11,000+ files describe that source collection, not the exact
canonical evaluation subset. See the root `ARTIFACT_MANIFEST.md` for the frozen
study counts and protocol boundaries.
